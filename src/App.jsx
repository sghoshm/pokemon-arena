...
function App() {
  ...
  const choosePokemon = (pokemon) => {
    ...
    setOpponentPokemon({
      ...randomOpponent,
      attacks: [...randomOpponent.attacks],
    });

    setMessage(
      `You chose ${pokemon.name}! Opponent sent out ${randomOpponent.name}`,
    );
  ...
  const playerAttack = (selectedAttack) => {
    ...
    const opponentHp =
      opponentPokemon.hp - playerDmaage >= 0
        ? opponentPokemon.hp - playerDmaage
        : 0;
  ...
  const opponentAttack = () => {
    ...
    const playerHp =
      playerPokemon.hp - opponentDmaage >= 0
        ? playerPokemon.hp - opponentDmaage
        : 0;
